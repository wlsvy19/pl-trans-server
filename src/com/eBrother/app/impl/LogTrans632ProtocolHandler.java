...
public class LogTrans632ProtocolHandler  implements ProtocolHandler, MBeanRegistration {

	 //   protected static org.apache.juli.logging.Log log
	 //       = org.apache.juli.logging.LogFactory.getLog(TransProtocol.class);
	protected static Logger log = Logger.getLogger(LogTrans632ProtocolHandler.class.getName());
	// ------------------------------------------------------------ Constructor

	static ParserWorker m_parser = null; // = new ParserWorker ();

    public LogTrans632ProtocolHandler() {
    ...
    static public String m_szinbound = "";
    ...
    public void init() throws Exception {
        ...
        log.info("eBrotherTransProtocol.init - " + getName());
        ...
        synchronized ( this ) {
			...
			log.info("ParserWorker.inbound = " + szindir );
			log.info("ParserWorker.outdir = " + szoutdir );
			log.info("ParserWorker.patternserver = " + szpatternserver );
			log.info("ParserWorker.patternmeta = " + szpatternmeta );
			
	        if ( m_parser == null ) { // && szpatternmeta != null && UtilExt.isFileExist(szpatternmeta )) {

				String [] args = new String [] { szpatternserver,  szpatternmeta, szindir, szoutdir };
				
				m_parser = new ParserWorker ( null, args);
				
				log.info("ParserWorker start" );
				Thread th = new Thread ( m_parser );
				th.start();
				
	        }
	        else {
	        	log.info("ParserWorker skip. Already run" );
        ...
        /*
		String resource1 = (String)getAttribute ("springset");

		// 
		ApplicationContext ctx = new FileSystemXmlApplicationContext (resource1);
		
		RestDAO rt = ( RestDAO) ctx.getBean("restDAO");
		
		String sqlid = "listtname";
    ...
    protected static class TransProtocolHandler implements Handler {
        ...
        protected LogTrans632ProtocolHandler proto;
        protected AtomicLong registerCount = new AtomicLong(0);
        protected RequestGroupInfo global = new RequestGroupInfo();

        protected ConcurrentLinkedQueue<TransWorker632> recycledProcessors = 
            new ConcurrentLinkedQueue<TransWorker632>() {
            protected AtomicInteger size = new AtomicInteger(0);
            public boolean offer(TransWorker632 processor) {
            ...
            public TransWorker632 poll() {
                TransWorker632 result = super.poll();
                if ( result != null ) {
        ...
        TransProtocolHandler(LogTrans632ProtocolHandler proto) {
            this.proto = proto;
        }

        public boolean process(Socket socket) {
            TransWorker632 processor = recycledProcessors.poll();
            try {
                ...
                LogTrans632ProtocolHandler.log.debug
                    (
            ...
            // above.
                ...
                LogTrans632ProtocolHandler.log.error ("TransProtocol.proto.error", e);
            } finally {
        ...
        protected TransWorker632 createProcessor() {
            TransWorker632 processor =
                new TransWorker632(proto.maxHttpHeaderSize, proto.endpoint);
            ...
            processor.init ( "pattern_server", this.proto.getAttribute("pattern_server").toString() );
            processor.init ( "pattern_meta", this.proto.getAttribute("pattern_meta").toString() );
            processor.init ( "outbound", this.proto.getAttribute("outbound").toString() );
            
            register(processor);
            return processor;
        }
        
        protected void register(TransWorker632 processor) {
        ...
        protected void unregister(TransWorker632 processor) {
            if (proto.getDomain() != null) {
                synchronized (this) {
                    try {
                        RequestInfo rp = processor.getRequest().getRequestProcessor();
